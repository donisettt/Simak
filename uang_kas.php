<?php 
  ...
  if (isset($_POST['btnAddBulanPembayaran'])) {
    if (addBulanPembayaran($_POST) > 0) {
      setAlert("Berhasil Menambahkan", "Bulan Pembayaran telah ditambahkan!", "success");
  ...
  if (isset($_POST['btnEditBulanPembayaran'])) {
    if (editBulanPembayaran($_POST) > 0) {
      setAlert("Berhasil Diubah", "Bulan pembayaran berhasil diubah!", "success");
      header("Location: uang_kas.php");
    }
  }
      ...
      <div class="container-fluid">
        ...
        <div class="row">
          <?php foreach ($bulan_pembayaran as $dbp): ?>
            ...
            $total_uang_kas_bulan_ini = mysqli_fetch_assoc(mysqli_query($conn, "
                SELECT 
                    SUM(IFNULL(minggu_ke_1, 0) + IFNULL(minggu_ke_2, 0) + IFNULL(minggu_ke_3, 0) + IFNULL(minggu_ke_4, 0)) 
                    AS total_uang_kas_bulan_ini 
                FROM uang_kas 
                WHERE id_bulan_pembayaran = '$id_bulan_pembayaran'
            "));
            ...
            <div class="col-lg-3">
              <div class="card shadow">
                <div class="card-body">
                  ...
                  <h6>Total Uang Kas Bulan Ini: <span class="my-2 btn btn-success">Rp. <?= number_format($total_uang_kas_bulan_ini ?? 0); ?></span></h6>
                  <a href="detail_bulan_pembayaran.php?id_bulan_pembayaran=<?= $dbp['id_bulan_pembayaran']; ?>" class="btn btn-info btn-sm"><i class="fas fa-fw fa-align-justify"></i></a>
                  <button type="button" data-toggle="modal" data-target="#editBulanPembayaranModal<?= $dbp['id_bulan_pembayaran']; ?>" class="btn btn-sm btn-success"><i class="fas fa-fw fa-edit"></i></button>
                  <!-- Modal -->
                      ...
                      <form method="post">
                        ...
                        <div class="modal-content">
                          ...
                          <div class="modal-body">
                            ...
                            <div class="col-lg">
                              <div class="form-group">
                                  <label for="nama_bulan<?= $dbp['id_bulan_pembayaran']; ?>">Nama Bulan</label><br>
                                  <select name="nama_bulan" id="nama_bulan" class="form-control">
                                    <option value="januari">Januari</option>
                                    <option value="februari">Februari</option>
                                    <option value="maret">Maret</option>
                          ...
                          <div class="col-lg">
                              <div class="form-group">
                                  ...
                                  <input type="number" id="tahun<?= $dbp['id_bulan_pembayaran']; ?>" name="tahun" value="<?= $dbp['tahun']; ?>" class="form-control">
                              </div>
                          </div>
                            </div>
                            <?php endif ?>
                  ...
                  ?>
                    <a href="hapus_bulan_pembayaran.php?id_bulan_pembayaran=<?= $dbp['id_bulan_pembayaran']; ?>" 
                      ...
                      data-nama="<?= ucwords($dbp['nama_bulan']); ?> | <?= $dbp['tahun']; ?>">
